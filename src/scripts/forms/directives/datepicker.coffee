  ...
  '$timeout', "agDate", ($timeout, agDate)->
    ...
    link: ($scope, $element, $attrs, ngModelCtrl) ->
      ...
      if !$attrs.agDatepicker?
        ...
        input = """<input name='#{$attrs.id || ""}' class='form-control' placeholder='#{$attrs.placeholder || ""}' #{"disabled" if $attrs.disabled?}>
  ...
  "agDate", (agDate) ->
    ...
    link: (scope, element, attrs, ngModelCtrl) ->
      ...
      ngModelCtrl.$parsers.push (viewValue) ->
        ...
        if isValid
          moment(viewValue, dateFormat).format(modelFormat)
        else
          ""
      ...
      ngModelCtrl.$formatters.push (modelValue) ->
        ...
        ngModelCtrl.$setValidity('dateFormat', isValid)
        moment(modelValue, modelFormat).format(dateFormat)
  ]
