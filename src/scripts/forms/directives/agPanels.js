    ...
    "$scope", function($scope) {
      ...
      var collapseGrid = function(element) {
        ...
        if ($scope.state === "normal") {
          if ($scope.gridRowNum) { gridEl.jqGrid("setGridParam", {rowNum: $scope.gridRowNum}).trigger("reloadGrid", [{page: 1}])
          } else {
            for (row of Array.from(angular.element(gridEl).find("tbody").children())) {
      ...
      var collapseForm = function(element) {
        ...
        if ($scope.state === "normal") {
          for (let el of Array.from(panelBody)) {
            if (angular.element(el).hasClass("ng-hide")) { angular.element(el).removeClass("ng-hide")
