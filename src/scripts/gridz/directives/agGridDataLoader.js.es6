...
gridz.service("agGridDataLoader", [
  "$log", "$http", "$document",
  ($log, $http, $document) => // Creates custom defined function for retrieving jgGrid data.
  // url - data source url
  // gridCtrl - grid controller instance
  (url, gridCtrl) => // see http://www.trirand.com/jqgridwiki/doku.php?id=wiki:retrieving_data
  (function(params, loadingDivSelector) {
    // load grid data
    ...
    return promise.finally(function() {
      ...
      if (!Array.from(pendingUrls).includes(url)) { return loadingEl.hide(); }
    });
  })
]);
