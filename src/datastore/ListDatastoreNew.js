...
const withRestApi = o => {
  ...
  let api = {
    ...
    get ky(){ return kyApi.ky},

    fetch({method = 'get', ...opts}){
      let { action = '' } = opts //can pass an 'action' to get appended to the end of url
      return o.ky(`${endpoint}${resource}`, {...opts, method}).json()
    }

  }

  return Object.assign(obj, api)
}

const pagerData = function(obj){
  //the current paged view of the data {data:[...], page: , records: , total: }
  let pager = {}

  // updates the dataView and the pager
  function updatePager(pagedData) {
    pager = pagedData
  }

  return Object.assign(obj, {updatePager})

}

const ApiSearch = function(obj){
  ...
  // return mix(Drone).it(o, ...opts).with(
  //   withRestApi
  // )
}
