    ...
    return {
      ...
      link: function(scope, elem, attrs) {
        ...
        // Tokenise input
          ...
          // Other variables
          var value, values
          var valid = true; var double = false
        ...
        if (attrs.customClass) {
          if (attrs.customClass.charAt(0) === '.') { attrs.customClass = attrs.customClass.substr(1) }
        ...
        attrs.$observe('data', function(val) {
          var string = val.trim(); var letter = ''
          ...
          if (attrs.charCount && !isNaN(attrs.charCount)) {
            var newString = string.split(/(?=[A-Z])/); var count = parseInt(attrs.charCount)
