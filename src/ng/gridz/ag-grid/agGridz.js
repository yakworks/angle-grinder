...
import agGridCtrl from './agGridzCtrl'
...
gridz.directive('agGridz',
  function($timeout, $parse, $q, GridDataLoader, ActionPopupHandler, pathWithContext) {
    const link = function(scope, element, attrs, gridCtrl) {
      ...
      // read grid options
      const optionsPromise = $parse(attrs.gridOptions)(scope)
...
<div class='tagged-input' style="min-height: 35px; margin-bottom: -4px">Drop headers here</div>
    ...
    return {
      ...
      <div class="gridz-wrapper">
        <ag-grid-toolbar action-ctrl="gridCtrl.actionCtrl"></ag-grid-toolbar>
