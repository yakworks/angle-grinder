(function() {
    ...
    forms.directive("agDeleteButton", function() {
        return {
            ...
            controller: [ "$scope", "$http", "$element", function($scope, $http, $element) {
                ...
                return $scope.$watch("confirmation", function(confirmation) {
                    ...
                    $scope.$watch(function() {
                        return $scope.deleting = $http.pendingRequests.length > 0;
                    });
                    if (confirmation) {
            ...
            template: '<button type="button" class="btn btn-danger ag-delete-button" ng-class="{disabled: deleting}"\n        ng-mouseleave="confirmation = false"\n        ng-click="delete()">\n  <i class="icon-trash"></i> {{label}}<span ng-show="deleting">...</span>\n</button>'
        };
    });
        ...
        return {
            ...
            controller: [ "$scope", "$http", function($scope, $http) {
                ...
                return $scope.$watch(function() {
                    return $scope.saving = $http.pendingRequests.length > 0;
                });
            } ],
