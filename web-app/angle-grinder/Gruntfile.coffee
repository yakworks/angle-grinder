...
module.exports = (grunt) ->
  ...
  grunt.initConfig
    ...
    watch:
      ...
      livereload:
        files: ["<%= yeoman.app %>/*.html", "{.tmp,<%= yeoman.app %>}/styles/*.css", "{.tmp,<%= yeoman.app %>}/scripts/*.js", "{.tmp,<%= yeoman.app %>}/widgets/**/*.js", "<%= yeoman.app %>/images/*.{png,jpg,jpeg}"]
        tasks: ["livereload"]
  ...
  grunt.registerTask "init", "moving things into place", ->
    compDir = "<%= yeoman.app %>/components"
    thirdParty = "<%= yeoman.app %>/third-party"
    grunt.config "copy.third-party.files", [
      expand: true
      flatten: true
      src: ["<%=cfg.comp%>/font-awesome/font/*"]
      dest: "<%=cfg.dist%>/font/"
    ,
      expand: true
      flatten: true
    ...
    ,
      ...
      cwd: "<%=cfg.comp%>"
      dest: "<%=cfg.vendor%>"
      src: ["angular/*.js", "angular-resource/*", "angular-ui/build/*", "angular-bootstrap/*", "angular-strap/dist/*", "select2/*", "select2-bootstrap-css/*"]
    ]

    #TODO replace the "images" urls for jquery moved css
    grunt.task.run "copy:third-party", "less:dist"
