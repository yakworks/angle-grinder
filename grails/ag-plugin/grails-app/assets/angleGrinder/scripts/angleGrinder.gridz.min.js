(function() {
    ...
    gridz.directive("agGrid", [ "$timeout", "$log", "$parse", "agGridDataLoader", "ActionPopupHandler", "pathWithContext", "camelize", function($timeout, $log, $parse, agGridDataLoader, ActionPopupHandler, pathWithContext, camelize) {
        ...
        link = function(scope, element, attrs, gridCtrl) {
            var alias, dropDownsection, gridEl, initializeGrid, options, timeoutPromise, unregister;
                ...
                if (options.filterToolbar) {
                    gridEl.jqGrid("filterToolbar", {
                        "beforeSearch": function() {
                            var criteria, defaultFilters, postData;
                            postData = gridEl.jqGrid("getGridParam", "postData");
                            defaultFilters = postData.defaultFilters || postData.criteria;
                            criteria = _.extend(JSON.parse(defaultFilters), _.pick(postData, function(value, key) {
                                return key !== "page" && key !== "criteria" && key !== "max" && key !== "sort" && key !== "order" && key !== "nd" && key !== "_search";
                            }));
...
(function() {
    ...
    AgGridCtrl = function(_super) {
        ...
        AgGridCtrl.prototype.search = function(criteria) {
            var deferred, params, promise;
            deferred = this.$q.defer();
            params = {
                "page": 1,
                "search": this.hasSearchFilters(criteria),
                "postData": {
            ...
            promise.then(function() {
                return deferred.resolve(criteria);
            });
            return deferred.promise;
        };
    ...
    gridz.directive("agGridQuickSearch", [ function() {
        return {
            ...
            "link": function($scope) {
                ...
                return $scope.search = function(criteria) {
                    return $scope.grid.search(criteria);
                };
            },
    ...
    gridz.directive("quickSearchButton", function() {
        return function(scope, element, attrs) {
            return element.bind("keydown", function(event) {
                ...
                if (event.which === 27) {
                    if (scope.criteria) {
                        scope.criteria.quickSearch = "";
                    }
                    scope.$apply();
                    return scope.search(scope.criteria);
                }
            });
        };
    });
...
(function() {
    var gridz;
    gridz = angular.module("angleGrinder.gridz");
    gridz.value("hasSearchFilters", function(criteria) {
        var value, _;
        for (_ in criteria) {
            value = criteria[_];
            if (value == null) {
    ...
    gridz.directive("agSearchButton", function() {
        return {
            "restrict": "E",
            "replace": true,
            "template": '<button type="submit" ng-click="advancedSearch(criteria)" ng-disabled="searching" class="btn btn-info">\n  <i class="fa fa-search fa-inverse"></i> Search<span ng-show="searching">...</span>\n</button>'
        };
    });
    gridz.directive("agResetSearchButton", function() {
        return {
            "restrict": "E",
            "replace": true,
            "template": '<button type="button" ng-click="resetSearch(criteria)" ng-disabled="searching" class="btn">\n  <i class="fa fa-times"></i> Reset<span ng-show="searching">...</span>\n</button>'
        };
    });
    gridz.directive("agSearchForm", [ "$log", function($log) {
        return {
            ...
            "controller": [ "$scope", "$parse", "$attrs", function($scope, $parse, $attrs) {
                ...
                $scope.advancedSearch = function(criteria) {
                    var form;
                    if (criteria == null) {
                        criteria = {};
                    }
                    ...
                    return gridSearch(criteria);
                };
                return $scope.resetSearch = function(criteria) {
                    var defaultFilters;
                    if (criteria == null) {
                        criteria = {};
                    }
