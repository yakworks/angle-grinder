...
(function() {
    ...
    gridz.directive("agGridXlsExport", [ "$window", "NotificationDialogServ", "$compile", "$filter", function($window, NotificationDialogServ, $compile, $filter) {
        return {
            "restrict": "A",
            "link": function(scope, element, attrs) {
                var exp, gridIterator;
                gridIterator = function(ws, range, format) {
                ...
                return element.on("click", function(event) {
                    var colMod, csvData, data, exclude, grid, headers, iframe, labels, wb, ws, wscols;
                    event.preventDefault();
                    grid = scope.$grid;
                    if (grid.getSelectedRowIds().length !== 0) {
                        if ($window.navigator.userAgent.indexOf("MSIE ") > 0 || !!$window.navigator.userAgent.match(/Trident.*rv\:11\./)) {
                            ...
                            data = _.map(grid.getSelectedRows(true), function(row) {
                                var result;
                                result = {};
                                _.each(colMod, function(v, k) {
                                    ...
                                    if (v.formatter && v.formatter.toString().indexOf("currency") > -1) {
                                        return result[key] = $filter("currency")(result[key], "");
                                    }
                                });
                                return result;
                            });
