(function() {
    ...
    gridz.directive("agGrid", [ "$timeout", "$log", "$parse", "agGridDataLoader", "ActionPopupHandler", "pathWithContext", "camelize", function($timeout, $log, $parse, agGridDataLoader, ActionPopupHandler, pathWithContext, camelize) {
        ...
        link = function(scope, element, attrs, gridCtrl) {
            var alias, dropDownsection, gridEl, initGroupCheckboxes, initializeGrid, options, timeoutPromise, unregister;
            ...
            initializeGrid = function() {
                var groupCheckBox, groupingView, onGridComplete, _gridComplete;
                ...
                if (options.filterToolbar) {
                    gridEl.jqGrid("filterToolbar", {
                        "beforeSearch": function() {
                            var defaultFilters, filters, postData;
                            postData = gridEl.jqGrid("getGridParam", "postData");
                            defaultFilters = postData.defaultFilters || postData.filters;
                            filters = _.extend(JSON.parse(defaultFilters), _.pick(postData, function(value, key) {
                                return key !== "page" && key !== "filters" && key !== "max" && key !== "sort" && key !== "order" && key !== "nd" && key !== "_search";
                            }));
            ...
            initGroupCheckboxes = function(gridId, checkboxSelector) {
                var headerSelector;
                headerSelector = "." + alias + "ghead_0";
                return $("#" + gridId).on("change", checkboxSelector, function(e) {
                    var checkboxes, currentCB, headers;
                    currentCB = $(this);
...
(function() {
    ...
    AgGridCtrl = function(_super) {
        ...
        AgGridCtrl.prototype.getSelectedRowIds = function(gridOrder) {
            var grid, ids;
            if (gridOrder == null) {
                gridOrder = false;
            }
            ids = this.getParam("selarrrow");
            if (gridOrder) {
                grid = this.getGridEl();
                return ids.sort(function(rowid1, rowid2) {
                    return grid.jqGrid("getInd", rowid1) - grid.jqGrid("getInd", rowid2);
                });
            } else {
                return ids;
            }
        };
        AgGridCtrl.prototype.getSelectedRows = function(gridOrder) {
            var getRowData, ids;
            if (gridOrder == null) {
                gridOrder = false;
            }
            getRowData = _.bind(this.getRowData, this);
            ids = this.getSelectedRowIds(gridOrder);
        ...
        AgGridCtrl.prototype.updateRow = function(id, data, emptyMissingCells) {
        ...
        AgGridCtrl.prototype.addAdditionalFooter = function(data) {
            var footerRow, k, newFooterRow, td, v, withoutComma, _results;
            footerRow = this.$element.find("tr.footrow");
            newFooterRow = void 0;
            ...
            for (k in data) {
                ...
                if (td.length > 0) {
                    ...
                    if (!isNaN(withoutComma)) {
                        _results.push(td[0].innerHTML = "<div class='pull-right currency-content'>" + v + "</div>");
                    } else {
                        _results.push(td[0].innerHTML = "<div class=''>" + v + "</div>");
                    }
                } else {
...
(function() {
    var Gridz, currencyUnformatter, _ref, _ref1, _ref2, __indexOf = [].indexOf || function(item) {
    ...
    Gridz = function() {
        ...
        Gridz.prototype.init = function(element, opts) {
            ...
            if (this.options.actionPopup && this.options.editable !== false) {
                this.addRowActionColumn();
            }
            if (this.options.editOndblClick && this.options.editable !== false) {
                this.editOndblClick();
            }
        ...
        Gridz.prototype.getOptions = function(options) {
            var optBeforeSelectRow, optOnSelectAll, optOnSelectRow, _gridComplete;
            ...
            options.beforeSelectRow = function(_this) {
                return function(rowid, e) {
                    ...
                    if ($.isFunction(optBeforeSelectRow)) {
                        resp = optBeforeSelectRow.apply(_this, arguments);
                    }
            ...
            options.onSelectRow = function(_this) {
                return function(rowid, isChecked, event) {
                    _this.onSelectRow.apply(_this, arguments);
                    if ($.isFunction(optOnSelectRow)) {
                        optOnSelectRow.apply(_this, arguments);
                    }
                    return true;
                };
            }(this);
            optOnSelectAll = options.onSelectAll;
            options.onSelectAll = function(_this) {
                return function(rowIds, status) {
                    _this.onSelectAll.apply(_this, arguments);
                    if ($.isFunction(optOnSelectAll)) {
                        optOnSelectAll.apply(_this, arguments);
                    }
                    return true;
                };
            }(this);
            ...
            options.gridComplete = function(_this) {
                return function() {
                    ...
                    _this.gridEl.trigger("gridComplete");
                    if (_this.options.multiSetSelection) {
                        return _this.memoizeSelectedRows();
                    }
                };
            }(this);
        ...
        Gridz.prototype.memoizeSelectedRows = function() {
            var selectedRows;
            selectedRows = this.selectedRowIds;
            return _.each(this.gridEl.jqGrid("getGridParam", "selarrrow"), function(id) {
                if (!(__indexOf.call(selectedRows, id) >= 0)) {
                    return selectedRows.push(id);
                }
            });
        };
        Gridz.prototype.onSelectAll = function() {
            if (this.options.multiSetSelection) {
                return this.selectedRowIds = [];
            }
        };
        Gridz.prototype.onSelectRow = function(rowid, isChecked, e) {
            var grid, ids, pager, rowNum, selected, selectedRows, span, text;
            if (this.gridEl.jqGrid("getGridParam", "agRowNumber")) {
                ids = this.gridEl.getDataIDs();
                text = "";
                if (this.gridEl.jqGrid("getGridParam", "selarrrow").length === 1) {
                    rowNum = (this.gridEl.jqGrid("getGridParam", "page") - 1) * this.gridEl.jqGrid("getGridParam", "rowNum") + ids.indexOf(rowid) + 1;
                    text = "Current row # " + rowNum + " | ";
                }
                pager = this.gridEl.parent().parent().parent().parent().find("#paymentGrid-pager_right");
            ...
            if (this.options.multiSetSelection) {
                ...
                if (e != null ? e.shiftKey : void 0) {
                    ...
                    _.each(selectedRows, function(id) {
                        if (!(__indexOf.call(selected, id) >= 0)) {
        ...
        Gridz.prototype.popupFormatter = function(containerId, rowClass, icon) {
            return '<a class="' + rowClass + '" data-toggle="popover" href="#"\n   data-container="#' + containerId + '"><i class="' + icon + '"></i></a>';
        };
        ...
        Gridz.prototype.popupSetup = function(columnName, innerHTML) {
        ...
        Gridz.prototype.actionPopupOnShow = function(clickoverEl) {
            ...
            menuEl.on("click", "li a.row_action_delete", function(_this) {
                return function(e) {
                    e.preventDefault();
                    return _this.gridEl.trigger("deleteAction", [ id, self ]);
                };
            }(this));
            return menuEl.on("click", "li a.row_action_mass_update", function(_this) {
                return function(e) {
                    e.preventDefault();
                    return _this.gridEl.trigger("massUpdateAction", []);
                };
            }(this));
        };
    ...
    $.extend($.fn.fmatter, {
        ...
        "dateTime": function(cellVal, options) {
            return window.columnAligner("date", window.agLocalDateTimeFilter(cellVal), options);
        },
        ...
        "okIcon": function(cellVal, options, rowdata) {
            if (cellVal) {
                return "<i class='fa fa-check'></i><span style='display: none'>true</span>";
            } else {
                return "<span style='display: none'>false</span>";
            }
        },
    ...
    $.extend((_ref1 = $.fn.fmatter) != null ? _ref1.okIcon : void 0, {
        "unformat": function(cellVal) {
            if (cellVal === "true") {
                return true;
            } else {
                return false;
    ...
    $.extend((_ref2 = $.fn.fmatter) != null ? _ref2.currencyOrZero : void 0, {
        "unformat": currencyUnformatter
    });
    window.columnAligner = function(type, content, options) {
        var _ref3;
        if (options != null ? (_ref3 = options.colModel) != null ? _ref3.align : void 0 : void 0) {
            return content;
        } else {
            return '<span class="' + type + '-content">' + content + "</span>";
        }
    };
}).call(this);
        ...
        return {
            ...
            "link": function($scope, element, attrs) {
                return $scope.resetSort = function() {
                    var colModel, column, columnName, disabledClassName, order;
                    columnName = attrs.defaultColumn ? attrs.defaultColumn : "id";
                    order = attrs.defaultOrder ? attrs.defaultOrder : "asc";
                    colModel = $scope.grid.getGridEl().getGridParam("colModel");
                    angular.forEach(colModel, function(column) {
...
(function() {
    ...
    gridz.directive("agGridXlsExport", [ "$window", "NotificationDialogServ", "$compile", function($window, NotificationDialogServ, $compile) {
        return {
            "restrict": "A",
            "link": function(scope, element, attrs) {
                var cellFormatter, exp, setCellType;
                cellFormatter = function(ws, range, format) {
                    var C, R, cell, _results;
                    if (format == null) {
                        format = "#,##0.00";
                    }
                    R = range.s.r;
                ...
                setCellType = function(ws, range, type) {
                    var C, R, cell, _results;
                    if (type == null) {
                        type = "n";
                    }
                ...
                return element.on("click", function(event) {
                    var colMod, csvData, currencyColumns, data, exclude, grid, headers, i, iframe, labels, selectedRows, wb, ws, wscols;
                    event.preventDefault();
                    grid = scope.$grid;
                    if (grid.getSelectedRowIds().length !== 0) {
                        if ($window.navigator.userAgent.indexOf("MSIE ") > 0 || !!$window.navigator.userAgent.match(/Trident.*rv\:11\./)) {
                            ...
                            headers = [];
                            i = 0;
                            _.remove(colMod, function(row) {
                                var _ref;
                                return row.hidden || (_ref = row.name, __indexOf.call(exclude, _ref) >= 0);
                            });
                            _.each(colMod, function(row) {
                                var range;
                                range = {
                                    "s": {
                                        "r": 1,
                                ...
                                wscols.push(row.width ? {
                                    "wpx": row.width
                                } : {
                                    "wch": labels[row.name].length + 3
                                });
                                if (row.formatter && row.formatter.toString().indexOf("currency") > -1) {
                                    range.s.c = i;
                                    range.e.c = i;
                            ...
                            data = _.map(selectedRows, function(row) {
                                var result;
                                result = {};
                                _.each(colMod, function(v, k) {
                                    var key, val, value;
                                    key = v["label"] || v["name"];
                                    key = key.toString();
                                    value = row[v["name"]];
                                    ...
                                    if (__indexOf.call(exclude, key) < 0) {
                                        if (isNaN(val) || val === "" || val == null || val.toString().indexOf("E") > 0) {
                                            return result[key] = val;
                                        } else {
                                            return result[key] = Number(val);
                                        }
                                    }
                                });
                                return result;
                            ...
                            _.each(currencyColumns, function(range) {
                                return cellFormatter(ws, range);
                            });
                            ws["!cols"] = wscols;
                            wb = XLSX.utils.book_new();
                            XLSX.utils.book_append_sheet(wb, ws, "Sheet 1");
                            return XLSX.writeFile(wb, "download.xlsx");
                        }
                    } else {
    ...
    gridz.factory("ActionPopupHandler", [ "$log", function($log) {
        return function(gridEl, scope, attrs) {
            ...
            gridEl.on("massUpdateAction", function(event) {
                var action;
                event.preventDefault();
                action = attrs.massUpdateAction ? attrs.massUpdateAction : "massUpdate";
        ...
        return {
            ...
            "controller": [ "$scope", "$parse", "$attrs", function($scope, $parse, $attrs) {
                ...
                $scope.advancedSearch = function(filters) {
                    var beforeSearch, beforeSearchResult, form, res;
                    if (filters == null) {
                        filters = {};
                    }
                    beforeSearch = null;
                    if ($attrs.beforeSearch) {
                        beforeSearch = $scope[$attrs.beforeSearch] || $scope.$parent[$attrs.beforeSearch];
                    }
...
(function() {
    var gridz;
    gridz = angular.module("angleGrinder.gridz");
    gridz.value("ApplyFormattersServ", function(colModel, formatters, unFormatters) {
        if (formatters == null) {
            formatters = {};
        }
        if (unFormatters == null) {
            unFormatters = {};
        }
        return _.map(colModel, function(column) {
            var formName, formatter, unformat;
            ...
            if (unformat != null) {
                return column.unformat = unformat;
            } else {
                return console.log("For proper work, please add unformatter for " + formName);
            }
        });
    });
}).call(this);
...
(function() {
    ...
    app.directive("gridCrud", [ "$controller", "$timeout", function($controller, $timeout) {
        return {
            ...
            "link": function(scope, element, attrs) {
                var clicks, controllerName, ctrlLocals, gridEl;
    ...
    this.GridCrudCtrl = function() {
        ...
        function GridCrudCtrl($scope, $element, $attrs, $parse, $log, resourceBuilder, $window, restrictResource, $uibModal, pathWithContext, $timeout) {
            ...
            showForm = function() {
                var defaultModalOptions, modalOptions;
                if ($scope.isModal) {
                    defaultModalOptions = {
            ...
            $scope.highlightCell = function(rowid, colname) {
                grid().getGridEl().jqGrid("setCell", rowid, colname, "", {
                    "border-color": "green",
                    "border-width": "thin",
                    "border-style": "double"
                });
            ...
            $scope.dblClick = function(rowid, iRow, iCol, e) {
                var colModel, options, _ref, _ref1;
                options = $scope["" + (e != null ? (_ref = e.currentTarget) != null ? _ref.id : void 0 : void 0)].getGridEl().getGridParam();
                if (options.editable === false) {
                    return;
                }
                colModel = options.colModel;
                $scope.columnNameForFocus = (_ref1 = colModel[iCol]) != null ? _ref1["name"] : void 0;
                editAction(rowid);
...
(function() {
    var ManageGridColumnsCtrl, app;
    app = angular.module("angleGrinder.gridz");
    ManageGridColumnsCtrl = function() {
        ManageGridColumnsCtrl.$inject = [ "$scope" ];
        function ManageGridColumnsCtrl($scope) {
            var colModel, element, gridEl, systemColumns;
            ...
            $scope.save = function() {
                var displayedColumns, hiddenColumns, newColumnsOrder;
                gridEl = $scope.grid.getGridEl();
                newColumnsOrder = [];
                displayedColumns = [];
                hiddenColumns = [];
    ...
    app.directive("agManageGridColumns", [ "$uibModal", "pathWithContext", function($uibModal, pathWithContext) {
        return {
            "restrict": "E",
            "transclude": true,
            "replace": true,
