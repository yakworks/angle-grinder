        ...
        return {
            ...
            "$get": [ "$filter", "isFalsy", function($filter, isFalsy) {
                return function(date, useTimeZone, format) {
                    ...
                    if (useTimeZone || useTimeZone === !"false") {
                        return moment(date).format(format);
                    } else if (date instanceof Date) {
                        return moment.utc([ date.getFullYear(), date.getMonth(), date.getDate(), date.getHours(), date.getMinutes(), date.getSeconds() ]).format(format);
                    } else {
                        dateWithoutTZ = date.split("+")[0].split("-")[0];
                        return moment.utc(dateWithoutTZ).format(format);
                    }
                };
            } ]
        };
...
(function() {
    ...
    NotificationDialogCtrl = function(_super) {
        ...
        NotificationDialogCtrl.inject("$scope", "$modalInstance", "$log", "message");
    ...
    app.run([ "$templateCache", function($templateCache) {
        return $templateCache.put("templates/dialogs/notification.html", '<div class="modal-body">{{message}}</div>\n\n<div class="modal-footer">\n  <button class="btn btn-primary" ng-click="close()">OK</button>\n</div>');
    } ]);
    NotificationDialog = function() {
        ...
        NotificationDialog.prototype.open = function(message) {
            this.$log.info("Opening notification dialog, message:", message);
...
(function() {
    var app;
    app = angular.module("angleGrinder.common");
    app.directive("agTrimTime", [ "$filter", function($filter) {
        return {
            "restrict": "A",
            "link": function(scope, element, attrs, ctrl) {
                var updateModel;
                updateModel = function() {
                    var elem, ngModelCtrl;
                    elem = attrs.ngModel ? element : angular.element(element[0].querySelector("[ng-model='" + attrs.agTrimTime + "']"));
                    if (elem) {
                        ngModelCtrl = elem.controller("ngModel");
                    }
                    if (ngModelCtrl) {
                        return ngModelCtrl.$parsers.push(function(viewValue) {
                            return $filter("agDate")(viewValue, false, "YYYY-MM-DD");
                        });
                    }
                };
                return attrs.$observe("ngModel", updateModel);
            }
        };
    } ]);
}).call(this);