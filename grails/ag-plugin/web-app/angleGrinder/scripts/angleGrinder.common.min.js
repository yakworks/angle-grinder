(function() {
    ...
    common = angular.module("angleGrinder.common", [ "ui.bootstrap.modal" ]);
        ...
        return {
            ...
            "$get": [ "$filter", "isFalsy", function($filter, isFalsy) {
                return function(date, useTimeZone, format) {
                    var GMTRegex, TZOffsetRegex, formattedDate, isApplyTimeZone, m, parseDate, zoneIndex;
                    if (useTimeZone == null) {
                        useTimeZone = false;
                    }
                    if (format == null) {
                    ...
                    isApplyTimeZone = function() {
                        return useTimeZone || typeof useTimeZone === "string" && useTimeZone === !"false";
                    };
                    GMTRegex = /GMT/;
                    TZOffsetRegex = /\d(\+|\-)\d{2}:?\d{2}/;
                    formattedDate = date;
                    if (isApplyTimeZone()) {
                        if (typeof date === "string" && date.search(GMTRegex) !== -1) {
                        ...
                        } else if (date.search(GMTRegex) !== -1) {
                            formattedDate = parseDate(new Date(date));
                        } else if (date.search(TZOffsetRegex) !== -1) {
...
(function() {
    ...
    app.service("confirmationDialog", [ "$modal", "$log", "$q", function($modal, $log, $q) {
        return {
...
(function() {
    ...
    NotificationDialog = function() {
        NotificationDialog.$inject = [ "$modal", "$log" ];
...
(function() {
    ...
    app.directive("agTrimTime", [ "$filter", "$timeout", "agTrimTimeFormat", function($filter, $timeout, agTrimTimeFormat) {
        return {
            "restrict": "A",
            "link": function(scope, element, attrs) {
                var updater;
                updater = function() {
                    var format, modelElement, ngModelCtrl, updateModel, _ref;
                    format = (_ref = attrs.agTrimTimeFormat) != null ? _ref : agTrimTimeFormat;
                    modelElement = attrs.ngModel ? element : element.find("[ng-model='" + attrs.agTrimTime + "']");
                    if (modelElement) {
                        ngModelCtrl = angular.element(modelElement).controller("ngModel");
                    }
                    updateModel = function() {
                        if (ngModelCtrl) {
                            return ngModelCtrl.$parsers.push(function(viewValue) {
                                return $filter("agDate")(viewValue, true, format);
                            });
                        }
                    };
                    ...
                    if (scope.$data) {
                        return scope.$watch("$data", function(newVal) {
                            if (newVal) {
                                return scope.$data = $filter("agDate")(scope.$data, true, format);
                            }
                        });
                    } else if (ngModelCtrl) {
                        return scope.$watch(attrs.agTrimTime, function(newVal) {});
                    }
                };
                if (!attrs.ngModel) {
...
(function() {
    var app;
    app = angular.module("angleGrinder.common");
    app.factory("embeddedJsonServe", [ "$document", function($document) {
