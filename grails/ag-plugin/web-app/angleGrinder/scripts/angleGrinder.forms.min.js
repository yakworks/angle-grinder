    ...
    forms.config([ "$provide", function($provide) {
        return $provide.decorator("editableDirectiveFactory", [ "$delegate", "validationMessages", function($delegate, validationMessages) {
            ...
            return function() {
                ...
                directive.compile = function(element, attrs) {
                    return function(scope, element, attrs, ctrl) {
                        var disabled, form, name, options, viewValue;
                        link.apply(this, arguments);
                        form = ctrl[1];
                        name = attrs.eName;
        ...
        return {
            ...
            "link": function($scope, $element, $attrs, ngModelCtrl) {
                ...
                if ($attrs.agDatepicker == null) {
                    ...
                    input = "<input name='" + ($attrs.id || "") + "' class='form-control' placeholder='" + ($attrs.placeholder || "") + "' " + ($attrs.disabled != null ? "disabled" : void 0) + '>\n<span class="input-group-addon"><i class="fa fa-calendar"></i></span>';
                    $element.append(input);
                }
                $element.on("dp.change", function(event) {
                    if (ngModelCtrl) {
                        return $timeout(function() {
                            if (event.date != null && event.date._d !== void 0) {
...
(function() {
    ...
    mixin.factory("MassUpdateMixin", [ "$log", "$parse", "$uibModal", "pathWithContext", "NotificationDialogServ", function($log, $parse, $modal, pathWithContext, NotificationDialogServ) {
        return function($scope, args) {
            var backdropClass, controller, extraParams, gridName, templateUrl, windowClass;
            if (args == null) {
                args = {};
            }
            gridName = args.gridName, templateUrl = args.templateUrl, controller = args.controller, 
            extraParams = args.extraParams, windowClass = args.windowClass, backdropClass = args.backdropClass;
            if (controller == null) {
                controller = "MassUpdateFormCtrl";
            }
            return $scope.massUpdate = function() {
                var grid, params, selectedIds;
...
(function() {
    ...
    gridz.directive("selectFill", [ "$http", "pathWithContext", "$parse", function($http, pathWithContext, $parse) {
        return {
            "restrict": "E",
            "replace": true,
            "priority": 2e3,
    ...
    gridz.directive("agSelect2Fill", [ "$http", "pathWithContext", "$parse", function($http, pathWithContext, $parse) {
        return {
            "restrict": "E",
            "replace": true,
            "scope": true,
            "controller": [ "$scope", "$element", function($scope, $element) {
                return $scope.fill = function() {
                    var model, result, select, selectEl;
                    selectEl = $element.parent().find(".select2-container");
                    ...
                    if (model($scope.$parent) != null && model($scope.$parent).length === result.length) {
                        result = [];
                    }
