(function() {
    ...
    forms.run([ "uibDatepickerConfig", "uibDatepickerPopupConfig", function(uibDatepickerConfig, uibDatepickerPopupConfig) {
        uibDatepickerConfig.showWeeks = false;
        uibDatepickerConfig.formatDay = "d";
        return uibDatepickerPopupConfig.showButtonBar = false;
    } ]);
...
(function() {
    ...
    forms.directive("editableDatepicker", [ "editableDirectiveFactory", "$filter", function(editableDirectiveFactory, $filter) {
        return editableDirectiveFactory({
            ...
            "inputTpl": '<input type="text" style=\'width: 135px\' class="form-control" ng-model="$data" datepicker-popup="MM/dd/yyyy" is-open="opened" ng-readonly="disabled">',
            "render": function() {
                var div;
                this.parent.render.call(this);
                div = angular.element('<div class="input-group"></div>');
                this.inputEl.wrap(div);
                return this.inputEl.after('<button type="button" class="btn btn-default input-group-addon" ng-click="open($event)" ng-disabled="disabled">\n  <i style="padding: 1px; margin-left: -7px" class="fa fa-calendar"></i>\n</button>');
            },
            "init": function() {
                    ...
                    return transclude(scope, function(clone) {
                        angular.forEach(clone, function(child) {
                            if (child.localName === "input") {
                                return angular.forEach(child.attributes, function(attribute) {
                                    if (attribute.name === "disabled") {
                                        return scope.disabled = attribute.value;
                                    }
                                });
                            }
                        });
...
(function() {
    var forms;
    forms = angular.module("angleGrinder.forms");
    forms.factory("formDialog", [ "$uibModal", "pathWithContext", function($modal, pathWithContext) {
        return {
...
(function() {
    var mixin;
    mixin = angular.module("angleGrinder.forms");
    mixin.factory("massUpdateMixin", [ "$log", "$parse", "$uibModal", "pathWithContext", function($log, $parse, $modal, pathWithContext) {
        return function($scope, args) {
...
(function() {
    var app;
    app = angular.module("angleGrinder.forms");
    app.directive("agMaxLines", [ "isFalsy", "$parse", function(isFalsy, $parse) {
        return {
            "require": "ngModel",
            "restrict": "A",
            "link": function(scope, elem, attrs, ngModelCtrl) {
