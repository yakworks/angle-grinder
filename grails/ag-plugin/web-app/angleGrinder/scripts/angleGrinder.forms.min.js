...
(function() {
    var FormDialogCtrl, forms, __hasProp = {}.hasOwnProperty, __extends = function(child, parent) {
    ...
    FormDialogCtrl = function(_super) {
        ...
        FormDialogCtrl.inject("$scope", "$rootScope", "$log", "$modalInstance", "serverValidationErrorsHandler as validator", "dialogOptions");
        FormDialogCtrl.prototype.initialize = function() {
            var grid, item, _ref;
            this.$scope.dialogOptions = this.dialogOptions;
            _ref = this.$scope.dialogOptions, item = _ref.item, grid = _ref.grid;
            this.$scope.item = item;
            this.$scope.createNew = !item.persisted();
            this.$scope.closeDialog = function(_this) {
                return function() {
                    _this.$log.info("[ag] closing the dialog");
                    return _this.$modalInstance.close(_this.$scope.item);
                };
            }(this);
            this.$scope.save = function(_this) {
                return function(form, item) {
                    ...
                    onSuccess = function(response) {
                        _this.$log.info("[ag] item has been updated/created", response);
                        grid.saveRow(response.id, response);
                        return _this.$scope.closeDialog();
                    };
                    onError = function(response) {
                        _this.$log.error("[ag] something went wrong", response);
                        return _this.validator(form, response, item.resourceName());
                    };
                    return item.save({
                        "success": onSuccess,
                        "error": onError
            ...
            return this.$scope["delete"] = function(_this) {
                return function() {
                    var onError, onSuccess;
                    onSuccess = function(response) {
                        _this.$log.info("[ag] item has been deleted", response);
...
(function() {
    var ConfirmationDialog, ConfirmationDialogCtrl, forms, __hasProp = {}.hasOwnProperty, __extends = function(child, parent) {
        for (var key in parent) {
            if (__hasProp.call(parent, key)) child[key] = parent[key];
        }
        function ctor() {
    ...
    ConfirmationDialogCtrl = function(_super) {
        ...
        ConfirmationDialogCtrl.register(forms);
        ConfirmationDialogCtrl.inject("$scope", "$modalInstance", "$log", "message");
        ConfirmationDialogCtrl.prototype.initialize = function() {
            return this.expose(this.$scope, "message", "close");
        };
        ConfirmationDialogCtrl.prototype.close = function(confirmed) {
            this.$log.info("Closing confirmation dialog", confirmed);
            return this.$modalInstance.close(confirmed);
        };
    ...
    forms.directive("agTab", [ "$log", "$location", "pathWithContext", function($log, $location, pathWithContext) {
        return {
            ...
            "link": function(scope, element, attrs, tabsetCtrl) {
                ...
                scope.loading = false;
                active = function() {
                    return scope.name != null && $location.search().tab === scope.name;
                };
