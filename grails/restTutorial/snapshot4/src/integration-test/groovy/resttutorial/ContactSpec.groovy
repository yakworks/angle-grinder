...
class ContactSpec extends Specification {
    ...
    void "check GET list request without params "() {
        when:
        RestResponse response = rest.get("http://localhost:${serverPort}/api/contacts")
    ...
    void "check GET list request with max parameter"() {
        ...
        RestResponse response = rest.get("http://localhost:${serverPort}/api/contacts?max=20")
    ...
    void "check GET by id"() {
        when:
        RestResponse response = rest.get("http://localhost:${serverPort}/api/contacts/1")
    ...
    void "check POST request"() {
        when:
        RestResponse response = rest.post("http://localhost:${serverPort}/api/contacts"){
            json([
    ...
    void "check PUT request"() {
        when:
        RestResponse response = rest.put("http://localhost:${serverPort}/api/contacts/101"){
            json([
    ...
    void "check inactivate endpoint"() {
        when:
        RestResponse response = rest.post("http://localhost:${serverPort}/api/contacts/2/inactivate")
