...
3. if we are adding it becuase of pluralization,  renaming contact to contacts,
then lets explain that thats the reason. we can also mention thats the problem with plrualizing as we wouldn’t need to do anything if we had not right?
...
1. Override `delete` method for the controller
2. Add separate endpoint for this action

For both cases we cann't use `@Resource` because it doesn't allow us to change the controller that is used for our resource.
So we need to create our own controller and extend it from RestDaoController wich gives us ability to customize actions
using DAOs.
...
It will look somethings like this: `"/contact"(resources: "contact")` it will add url mappings for our controller.
