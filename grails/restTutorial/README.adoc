...
/contacts , GET , index , Paged List
/contacts/create, GET , create , Contact.newInstance() unsaved
/contacts, POST , save , The successfully saved contact (same as show's get)
/contacts/${id}, GET , show , The contact for the id
/contacts/${id}/edit, GET , edit , The contact for the id. same as show
/contacts/${id}, PUT , update , The successfully updated contact
/contacts/${id}, DELETE , delete , Empty response with HTTP status code 204
|===
...
//TODO We talked about his on slack, its not included and is not relevant in grails 3.
// Show the dependendcie line that allows this
...
//TODO provide a link that opens new tab/window to the spec in snapshot1

//TODO Explain the default GSON files below
...
===== error.gson
//TODO provide a link
This is for XXX. As you can see this is where the 422 status code gets set, etc...
...
We will setup a concrete implementation of a dao for the contact.
...
//TODO do we not need transactional annotation? Explain that they are transactional by default if so
...
So we need to add a new end point so a user can inactivate the contact instead of a delete

For that we will remove the `@Resource` annotation from the contacts domain and add the ContactController.groovy, but
`@Resource` made a one more feature for us - updating urlMappings, so now we need to add our url by hands.
It would like somethings like this: `"/contacts"(resources: "contact")` it will add url mappings for our controller.
...
The `ContactController` we will extend from `RestDaoController` and add our `inactivate` action:
...
For adding UI we will use awesome https://github.com/9ci/angle-grinder[Angle-Grinder] plugin that helps to integrate Angular
with Grails.
...
void "check DELETE request"() {
        when:
        //TODO in the tests lets move the url to a class level var for the baseUrl so we only need to change it one place
...
and setup default url mapping for it:
//TODO lets leace ths out for now as to not add confusion
...
//TODO app should run at this point? Can do a list? Show a screen shot and let the use know
==== DELETE
