...
===== DELETE:
----
curl -i -X DELETE -H "Content-Type: application/json"  localhost:8080/contacts/50
HTTP/1.1 204
X-Application-Context: application:development
Content-Type: application/json;charset=UTF-8
...
----
curl -i -X POST -H "Content-Type: application/json" -d '{"lastName": "Cool"}' localhost:8080/contacts
HTTP/1.1 422
X-Application-Context: application:development
Content-Type: application/json;charset=UTF-8
Transfer-Encoding: chunked
...
===== 404 - Get Error:
----
curl -i -X GET -H "Content-Type: application/json"  localhost:8080/contacts/105
HTTP/1.1 404
X-Application-Context: application:development
Content-Type: application/json;charset=UTF-8
...
----
curl -i -X GET -H "Accept: application/xml"  http://localhost:8080/contacts/8
HTTP/1.1 406
X-Application-Context: application:development
Content-Length: 0
...
so you cant call `http://localhost:8080/contacts` , but we can use `serverPort` variable instead. And to make it more
...
We need to add `@Transactional` because services are not transactional by default starting from Grails 3.
...
delete action to make set inactive flag, then how we will anable it. So we will add a new end point so a user can
...
void "check inactivate endpoint"() {
        when:
        RestResponse response = rest.delete("${baseUrl}/contacts/2/activate")
