...
assets-pipeline plugin. First we need to create `assents` folder in `grails-app` and add `javascript` and
...
`setupData` adds count of entities to list and  `.jsonData` formats output for JS grid.
