...
[source,groovy]
----
//TODO show the package and the imports
@Integration
@Rollback //TODO why is this needed? does it really work?
class ContactGebSpec extends GebSpec {
    ...
    void "check POST request"() {
        ...
        response.status == 201
        //TODO
        JSON(??or what ever it is now) json = response.json
        json.with{
          id == 101
          firstName == "Test contact"
          lastName == null
...
More tests examples are in the are in the snapshot's `ContactGebSpec.groovy`
