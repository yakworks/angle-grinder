...
To avoid writting `nullable: true` billion times let's set the defaults to allow nulls for domains fields by default,
just add next code to `grails-app/config/application.groovy`
[source,groovy]
----
...
So now we need to add a domain class:
...
=== Functional GEB
The next step is to add tests for our app, to make it a bit more interesting we will geb tests:

First we need to add geb dependencies to `build.gradle`:
[source]
----
/****** GEB *******/
...
and to be able to pass `geb.env` property to run tests in different browsers add next code to the bottom of the file:
...
Also we need to add external configuration for geb tests, `src/integration-test/groovy/GebConfig.groovy`:
[source,groovy]
----
baseNavigatorWaiting = true
atCheckWaiting = true
quitCachedDriverOnShutdown = true //keeps the browser open after test if FALSE
...
To test the REST endpoints `RestBuilder` will be used.

[source,groovy]
----
void "check POST request"() {
        given:
        RestBuilder rest = new RestBuilder() // create RestBuilder
        when:
...
More tests are in `src/integration-test/groovy/geb/ContactGebSpec.groovy`
...
You can run with other browser, just replace `chrome` on `ie`/ `phantom`/`firefox`.

The final result of the first stage you can check at `angle-grinder/grails/restTutorial/snapshot1/result`

== Adding the DAO plugin for REST
...
Add in the dependence for the plugin
----
show the dep line
----

=== RestDaoController
Alexey, see http://mrhaki.blogspot.com/2014/07/grails-goodness-custom-controller-class.html

Add the superClass property to the Resource annotation. The dao plugin will setup a default DAO for every domain.

```groovy
@Resource(uri = '/contacts', formats = ["json"], superClass = RestDaoController)
class Contact {
  ...
}
```

Check that our functional tests still pass with the defaults.

=== Implementing the ContactDao

Use Case: Lets say we want to customize the insert to allow a user to pass in a name and have it be split into first and last names.

so our sanity test could look like

```groovy
def "test name splits into first and last"(){
  when:
  def resp = restInsert('{"name":"Joe Cool}')
  expect:
  resp.firstName = 'Joe'
  resp.lastName = 'Cool'
}
...
=== Snapshot 2 sample app

=== Implementing a RestDaoController

Just as you might do with the RestfulController as outlined in the docs here
http://docs.grails.org/3.2.x/guide/webServices.html#extendingRestfulController[extendingRestfulController]
, we can implement a RestDaoController when the defaults
using the Resource annotation and the DAO won't work for us, such as adding a new REST end point.
