...
Check https://github.com/basejump/grails3-geb-example to see how to configure geb tests for Grails3

Functional we will put into separate folder `geb` in the integration tests.

For testing we need http://grails.org/plugin/rest-client-builder[Rest Client Builder plugin], it is already integrated to
Grails3 so we do not need any dependencies changes.
...
In the `snapshot2` directory is final result that we got after "Creating a Basic Grails REST API" chapter, it will be a
...
For the next stage please move to next snapshot.
