describe "BaseCtrl", ->
  ...
  describe "expose entitities to the scope", ->
    ...
    beforeEach ->
      class MyCtrl extends BaseCtrl
        ...
        initialize: ->
          @expose @$scope, "foo", "doFoo"

          @$scope.bar = "bar"

        doFoo: -> @$scope.called = true

      module "testApp"

    it "exposes given entities to the scope", inject ($rootScope, $controller) ->
      $scope = $rootScope.$new()
