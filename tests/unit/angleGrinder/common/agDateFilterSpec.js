...
describe("agDateFilterSpec", function() {
  ...
  return describe("filter: agDateFilter", function() {
    ...
    beforeEach(inject($filter => filter = $filter("agDate"))
    );

    it("by default uses `short` date format", function() {
      const date = new Date(2014, 5, 12);

      return expect(filter(date)).to.eq("Jun 12, 2014");
    });
    ...
    return describe("filter config provider", () => it("can set the default date format", function() {
      const date = new Date(2014, 5, 14);

      provider.setDefaultFormat("dddd, MMMM DD, YYYY");
      return expect(filter(date, true)).to.eq("Saturday, June 14, 2014");
    }));
  });
});
